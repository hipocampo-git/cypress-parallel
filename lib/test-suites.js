...
async function getTestSuitePaths() {
  ...
  console.log('here 1');
  console.log(settings.testSuitesPath);
/*  let fileList;
  if (isPattern) {*/
...
function distributeTestsByWeightNew(testSuitePaths) {
  let specWeights = {};
  try {
    specWeights = JSON.parse(fs.readFileSync(settings.weightsJSON, 'utf8'));
  } catch (err) {
  ...
  // Assign specs by group
  for (const [key, value] of map.entries()) {
    threads[value % settings.threadCount].list.push(key);
  }



  // Sort the threads by accumulated weight
  // for (const [key, value] of map.entries()) {
  //   threads.sort((w1, w2) => w1.weight - w2.weight);
  //   threads[0].list.push(key);
  //   threads[0].weight += +value;
